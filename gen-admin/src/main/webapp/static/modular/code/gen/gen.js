...
GEN.getDataBases = function (id) {
    ...
    GEN.getData('/code/queryDatabases', 'databases');
    GEN.genBtnAble();
}
...
GEN.review = function () {
    ...
    GEN.param.codePackage = $("#codePackage").val();
    GEN.param.entityName = $("#className").val();
    window.open('/code/review_code?' + $.param(GEN.param),"_blank");

}

// 预览代码Json
GEN.pushEolink = function () {
    if (!this.validate()) {
        return;
    }
    GEN.param.tableId = $("#tableId").val();
    ...
    GEN.param.codePackage = $("#codePackage").val();
    GEN.param.entityName = $("#className").val();
    var data = $.getJSON('/code/push_eolink?' + $.param(GEN.param));
    console.info(data);
    alert("推送成功");
}
